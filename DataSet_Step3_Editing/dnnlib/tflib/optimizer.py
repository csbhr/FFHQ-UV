...
class Optimizer:
    ...
    def __init__(
            ...
            minibatch_multiplier:
        TfExpressionEx = None,  # Treat N consecutive minibatches as one by accumulating gradients.
    ...
    def _get_device(self, device_name: str):
        ...
        with tfutil.absolute_name_scope(self.scope + "/Devices"), tf.device(device_name), tf.control_dependencies(None):
            ...
            if self.use_loss_scaling:
                device.loss_scaling_var = tf.Variable(np.float32(self.loss_scaling_init),
                                                      trainable=False,
        ...
        if self._report_mem_usage:
            ...
            try:
                with tf.name_scope(self.id + '_mem'), tf.device(device.name), tf.control_dependencies([loss]):
                    deps.append(
                        autosummary.autosummary(self.id + "/mem_usage_gb",
            ...
            with tfutil.absolute_name_scope(self.scope + "/Broadcast"), tf.device(None):
                for all_vars in zip(*[device.grad_clean.keys() for device in self._devices.values()]):
                    if len(all_vars) > 0 and all(
                            dim > 0
                            for dim in all_vars[0].shape.as_list()):  # NCCL does not support zero-sized tensors.
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with tfutil.absolute_name_scope(self.scope + "/Apply%d" % device_idx), tf.device(device.name):
                ...
                if self.minibatch_multiplier is None:
                    ...
                    with tf.control_dependencies(None):
                        for var in device.grad_clean.keys():
                            device.grad_acc_vars[var] = tf.Variable(tf.zeros(var.shape),
                                                                    trainable=False,
                                                                    name="grad_acc_var")
                ...
                if device_idx == len(self._devices) - 1:
                    ...
                    all_ops.append(
                        autosummary.autosummary(self.id + "/overflow_frequency",
                                                tf.where(all_ok, 0, 1),
                                                condition=acc_ok))
                    if self.use_loss_scaling:
